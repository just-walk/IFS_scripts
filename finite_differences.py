...
def get_mat_fd_d2_o4(size,dx,plot_matrix=False):
    """Creates matrix for centered finite difference, second derivative, 4th order.
    size: size of (number of elements in) quantity to be differentiated
    dx: grid spacing (for constant grid)."""

    prefactor=1.0/(12.0*dx**2)
    mat=np.zeros((size,size),dtype='float')    
    for i in range(size):
        if i-1 >= 0:
            mat[i,i-1]=16
...
def fd_d1_o4(var,grid,mat=False):
    ...
    if not mat:
        mat=get_mat_fd_d1_o4(var.shape[-1],grid[1]-grid[0])
...
def fd_d2_o4(var,grid,mat=False):
    """Centered finite difference, first derivative, 4th order.
    var: quantity to be differentiated.
    grid: grid for var 
    mat: matrix for the finite-differencing operator. if mat=False then it is created"""
    ...
    if not mat:
        mat=get_mat_fd_d2_o4(var.shape[-1],grid[1]-grid[0])
